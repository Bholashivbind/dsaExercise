# we can use a map to solve this problem in lenear time. The idea is to create an empty map to store the first subarray's ending index, having a given sum. We traverse the given array and maintain the sum of elements seen so far.
...
def findMaxLenSublist(nums, S):
    ...
    # insert (0,-1) pair into the set to handle the case when a sublist with sum `S` starts from index 0
