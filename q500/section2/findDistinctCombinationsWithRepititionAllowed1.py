...
def findCombinations(A,k,i=0,out=[]):
    ...
    if len(out) == k:
        print(out)
        return
    # start from the previous element in the combination till the last element
    j = i
    while j < len(A):

        # add current element `A[j]` to the solutions and recur with the same index `j`
        out.append(A[j])
...
if __name__=='__main__':
    ...
    # if the list contains repeted elements, sort the list to handle duplicates combinations
    A.sort()
